...
function start2(temporaryDirectory, done) {
...
function start(temporaryDirectory, done) {
  if (temporaryDirectory) {
    start2(temporaryDirectory, done);
  } else {
    temporaryDirectory = getTmpDir();
    mkdirp.sync(temporaryDirectory);

    start2(temporaryDirectory, (err, config, cleanup) => {
      if (err) {
        return done(err);
      }
        ...
        process.on("beforeExit", () => {
          if (!deleting) {
            deleting = true;
            rimraf(temporaryDirectory, cleanup);
          }
        });
      });
    });
